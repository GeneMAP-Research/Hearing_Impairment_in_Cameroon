...
from bokeh.models import Legend, HoverTool, LegendItem, WheelZoomTool, ColumnDataSource, Span
from bokeh.transform import linear_cmap
from bokeh.palettes import inferno, Spectral6, Plasma
from bokeh.models.layouts import TabPanel, Tabs  # For python3
...
plot = figure(title=title, toolbar_location="above", x_axis_label="PC"+str(which_pcs[0]), y_axis_label="PC"+str(which_pcs[1]), width= 1450, height= 800,
...
plot.legend.label_height = 1
plot.legend.label_text_color = "#222222"
plot.legend.border_line_color = 'white'
plot.legend.margin = 1
...
PC1vs2_panel = TabPanel(child=plot, title="PC"+str(which_pcs[0])+" vs "+"PC"+str(which_pcs[1]))
...
export_svgs(tabs, filename=out+".svg")
export_png(tabs, filename=out+".png")
#show(tabs)
#exit()
...
# Create two panels, one for each conference
PC3vs4_panel = TabPanel(child=plot, title="PC"+str(which_pcs[0])+" vs "+"PC"+str(which_pcs[1]))
...
# Create two panels, one for each conference
PC2vs3_panel = TabPanel(child=plot, title="PC"+str(which_pcs[0])+" vs "+"PC"+str(which_pcs[1]))
...
# Create two panels, one for each conference
PC5vs6_panel = TabPanel(child=plot, title="PC"+str(which_pcs[0])+" vs "+"PC"+str(which_pcs[1]))
...
# Create two panels, one for each conference
PC7vs8_panel = TabPanel(child=plot, title="PC"+str(which_pcs[0])+" vs "+"PC"+str(which_pcs[1]))
...
# Create two panels, one for each conference
PC9vs10_panel = TabPanel(child=plot, title="PC"+str(which_pcs[0])+" vs "+"PC"+str(which_pcs[1]))

# Assign the panels to Tabs
tabs = Tabs(tabs=[PC1vs2_panel, PC3vs4_panel, PC2vs3_panel, PC5vs6_panel, PC7vs8_panel, PC9vs10_panel])
